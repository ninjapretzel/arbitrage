	...
	class Program {
		...
		static void Main(string[] args) {
			...
			foreach (string line in lines) {
				...
				if (!graph.ContainsKey(from)) { graph[from] = new Dictionary<string, double>(); }
				if (!graph.ContainsKey(to)) { graph[to] = new Dictionary<string, double>(); }
			...
			while (queue.Count > 0) {
				...
				foreach (var pair in graph) {
					...
					if (!data.path.Contains(pair.Key)) {
						...
						if (final.rate > 1) { profitable.Add(final); }

						// Ignore anywhere we lose money, even if it's still profitable
						if (final.rate >= compare.rate) { queue.Enqueue(next); }
					}
				}
			}
